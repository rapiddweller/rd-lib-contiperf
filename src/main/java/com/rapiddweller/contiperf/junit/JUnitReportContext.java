		...
		if (suite != null) {
		    for (Field field : suite.getClass().getFields()) {
	    		try {
	    			...
	    			else if (com.rapiddweller.contiperf.ExecutionLogger.class.isAssignableFrom(field.getType()))
	    				modules.add(new LoggerModuleAdapter((com.rapiddweller.contiperf.ExecutionLogger) field.get(suite)));
	            } catch (Exception e) {
